...
def load_products():
    ...
    # GitHub/Streamlit Cloud에서는 CSV의 한글 인코딩이 환경에 따라 달라질 수 있으므로
    # UTF-8 계열부터 CP949/EUC-KR까지 순서대로 자동 시도합니다.
    ...
    if df is None:
        st.error('products.csv 파일을 읽을 수 없습니다. CSV 파일의 인코딩을 확인해주세요.')
        with st.expander('상세 오류 보기'):
            st.code('\n'.join(read_errors))
        st.stop()

    # 한글 CSV의 BOM이나 불필요한 공백 때문에 열 이름을 못 찾는 문제를 방지합니다.
